#!/bin/bash

# Главный скрипт установки окружения
# Запускается внутри VM после провижионинга

set -e  # Прерывать выполнение при ошибках

echo "=================================================="
echo "🚀 Начинаем установку окружения разработки"
echo "=================================================="

DEBIAN_FRONTEND=noninteractive

# Подготовка: конвертация файлов Windows -> Unix
echo ""
echo "🔧 Подготовка файлов (конвертация CRLF -> LF)..."
sudo DEBIAN_FRONTEND=noninteractive apt-get update -qq
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y dos2unix

# Конвертируем все скрипты и env файлы
find /vagrant/vm -type f -name "*.sh" -exec dos2unix {} \; 2>/dev/null || true
if [ -f /vagrant/vm/env ]; then
  dos2unix /vagrant/vm/env 2>/dev/null || true
fi

echo "✅ Файлы подготовлены"

# Загружаем переменные из vm/env
set -a
source /vagrant/vm/env
set +a

SCRIPTS_DIR="/vagrant/vm/scripts"

# 1. Обновление системы
echo ""
echo "📦 Обновление системы..."
sudo DEBIAN_FRONTEND=noninteractive apt update

# 2. Установка базовых пакетов
echo ""
echo "🔧 Установка базовых пакетов..."
bash "${SCRIPTS_DIR}/install-base.sh"

# 3. Установка Docker
echo ""
echo "🐳 Установка Docker Engine..."
bash "${SCRIPTS_DIR}/install-docker.sh"

# 4. Установка LXC
echo ""
echo "📦 Установка LXC с встроенным dnsmasq..."
bash "${SCRIPTS_DIR}/install-lxc.sh"

# 5. Настройка SSH
echo ""
echo "🔐 Настройка SSH сервера..."
bash "${SCRIPTS_DIR}/setup-ssh.sh"

# 6. Настройка SSHFS (если указан)
if [[ -n "${SSHFS_URL}" ]]; then
    echo ""
    echo "💾 Настройка SSHFS подключения..."
    bash "${SCRIPTS_DIR}/install-sshfs.sh"
else
    echo ""
    echo "ℹ️  SSHFS_URL не указан, пропускаем монтирование"
fi

# 7. Клонирование и настройка проекта (если указан)
if [[ -n "${GIT_FULL_URL}" ]]; then
    echo ""
    echo "📥 Клонирование Git проекта..."
    bash "${SCRIPTS_DIR}/setup-project.sh"
else
    echo ""
    echo "ℹ️  GIT_FULL_URL не указан, пропускаем клонирование"
fi

# 8. Очистка после установки (кэши, временные файлы)
echo ""
bash "${SCRIPTS_DIR}/cleanup.sh"

# 9. Применение кастомных конфигов
bash "${SCRIPTS_DIR}/apply-configs.sh"

# 10. Финальные действия
echo ""
echo "=================================================="
echo "✅ Установка успешно завершена!"
echo "=================================================="
echo ""
echo "📌 Информация:"
echo "  • Docker:  $(docker --version)"
echo "  • Compose: $(docker compose version)"
echo "  • LXC:     $(lxc-ls --version 2>/dev/null || echo 'установлен')"
echo ""
echo "🔗 Подключение:"
echo "  • SSH: vagrant ssh"
echo "  • SOCKS5 прокси: vagrant ssh -- -D7777"
echo ""
echo "📂 Проекты находятся в: /home/vagrant/projects/"
echo ""
echo "⚠️  ВАЖНО: Для применения прав группы docker,"
echo "   переподключитесь через: vagrant ssh"
echo ""

# Перезагрузка shell для применения групп
exec /bin/bash
